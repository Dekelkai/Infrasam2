...
class RandomUniformSampler(VOSSampler):
    ...
    def sample(self, video, segment_loader, epoch=None):
        ...
        # print(f"[调试] Video: {video.video_name}, total frames in object: {len(video.frames)}")
        # print(video.frames)
        # for i, f in enumerate(video.frames):
        #     print(f"测试    Frame {i}: frame_idx={f.frame_idx}, path={f.image_path}")
        ...
        for retry in range(MAX_RETRIES):
            ...
            if isinstance(loaded_segms, LazySegments):
                ...
                for object_id, segment in segment_loader.load(
                    ...
                    # print(f"[调试] object_id={object_id}, segment_sum={segment.sum().item()}")
